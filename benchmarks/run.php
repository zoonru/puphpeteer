...
function sample(int $rootPid): array
{
    ...
    if (0 !== $result['code']) {
        throw new RuntimeException('ps failed: ' . $result['stderr']);
    }
    $rows = [];
    foreach (explode("\n", trim($result['stdout'])) as $line) {
        ...
        if (str_contains($cpu, '-')) {
            [$days, $cpu] = explode('-', $cpu, 2);
            $daySeconds = (float) $days * 86400;
        }
        foreach (explode(':', $cpu) as $part) {
...
function runTrial(int $trial, int $iterations): array
{
    ...
    try {
        ...
        $sampler = async(static function () use (&$stopSampler, $child, &$active, &$started, &$seen, &$peakRss, &$steadyPeakRss, &$samples, &$samplingErrors): void {
            while (!$stopSampler) {
                ...
                try {
                    ...
                    if ($active) {
                        $steadyPeakRss = max($steadyPeakRss, $rss);
                        ++$samples;
                    }
                    foreach ($rows as $row) {
                        if ($active || !$started) {
        ...
        try {
            $result = ProcessRunner::collect($child, 120, static function (string $chunk) use (&$pending, &$active, &$started, &$startCpu, &$seen, &$measurement): void {
                ...
                while (($end = strpos($pending, "\n")) !== false) {
                    ...
                    if ('BENCH_READY' === $line && !$started) {
                        $active = $started = true;
                        foreach ($seen as $pid => $row) {
                            $startCpu[$pid] = $row['cpu'];
                        }
        ...
        if (0 !== $result['code'] || null === $measurement) {
        ...
        if ('' !== $samplingErrors || 0 === $samples) {
            throw new RuntimeException('Resource sampler failed or collected no steady-state samples; inspect raw benchmark logs.');
        }
        $steadyCpu = 0.0;
        foreach ($seen as $pid => $row) {
            $steadyCpu += max(0, $row['cpu'] - ($startCpu[$pid] ?? 0)) * 1000;
        }
        $hasTime = preg_match(PHP_OS_FAMILY === 'Darwin' ? '/([\d.]+) real\s+([\d.]+) user\s+([\d.]+) sys/' : '/PUPHPETEER_TIME ([\d.]+) ([\d.]+) ([\d.]+)/', $stderr, $time);
        if (!$hasTime) {
            throw new RuntimeException('Cannot parse /usr/bin/time output; inspect raw benchmark logs.');
        }
...
try {
    if (!in_array(PHP_OS_FAMILY, ['Darwin', 'Linux'], true)) {
        throw new RuntimeException('Resource benchmark requires macOS or Linux with /usr/bin/time and ps.');
    }
    ...
    if (false === $trials || false === $iterations) {
        throw new InvalidArgumentException('--trials and --iterations must be positive integers');
    }
    $runs = [];
