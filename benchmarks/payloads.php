...
foreach ($sizes as &$size) {
    if (!ctype_digit($size) || (int) $size < 1 || (int) $size > ('stream' === $mode ? 32 : 12)) {
...
function measure(callable $run, array $expected): array
{
    ...
    try {
        ...
        EventLoop::cancel($heartbeat);
    }
}
$client = 'browser' !== $mode ? LargePayload::client() : null;
...
try {
    ...
    foreach ($sizes as $size) {
        ...
        foreach ($cases as $name => [$run, $expected]) {
            ...
            foreach (['ms', 'cpu_ms', 'mib_s'] as $metric) {
                $row[$metric] = array_sum(array_column($samples, $metric)) / $trials;
            }
            $row['max_gap_ms'] = max(array_column($samples, 'max_gap_ms'));
            $summary[] = $row;
            if (!isset($options['json'])) {
                fwrite(STDERR, $name . ' ' . $row['mib'] . " MiB: SHA-256 PASS\n");
            }
        }
    }
} finally {
...
if (isset($options['json'])) {
    echo json_encode(['environment' => $metadata, 'runs' => $runs, 'summary' => $summary], JSON_PRETTY_PRINT | JSON_THROW_ON_ERROR), "\n";
} else {
