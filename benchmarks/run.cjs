...
if (process.platform !== 'darwin') throw new Error('Resource benchmark requires macOS /usr/bin/time -l.');
if (!Number.isInteger(trials) || trials < 1) throw new Error('BENCH_TRIALS must be a positive integer.');
const output = path.join(__dirname, 'results/current/raw');
...
async function runTrial(backend, trial, fixtureURL) {
  const browser = await puppeteer.launch({executablePath: process.env.CHROME_BIN, headless: true, args: ['--no-proxy-server'], protocolTimeout: 15000});
  try {
    const args = ['-l', process.env.PHP_BIN || 'php', '-n'];
    args.push('-d', `extension=${extension}`);
    args.push(path.join(__dirname, 'benchmark.php'), backend);
...
(async () => {
  ...
  try {
    for (let trial = 0; trial < trials; trial++) {
      const backend = 'quickjs';
      console.log(`Running ${backend} ${trial + 1}/${trials}`);
      const run = await runTrial(backend, trial, `http://127.0.0.1:${fixture.address().port}/`);
      runs.push(run);
      fs.writeFileSync(path.join(__dirname, 'results/current/benchmark.json'), JSON.stringify({platform: os.platform(), arch: os.arch(), cpus: os.cpus()[0].model, timestamp: new Date().toISOString(), runs}, null, 2) + '\n');
