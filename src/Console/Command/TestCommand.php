...
final class TestCommand extends ProcessCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($suites as $current) {
            ...
            $results[] = ['suite' => $current, 'status' => 0 === $result['code'] ? 'ok' : 'error', 'code' => $result['code'], 'elapsed' => $result['elapsed']];
            if (!$this->jsonOutput) {
                $io->progressAdvance();
            }
            if (0 !== $result['code']) {
        ...
        if ($this->jsonOutput) {
            $this->writeJson($output, ['command' => $this->getName() ?? 'test', 'status' => 'ok', 'results' => $results]);
