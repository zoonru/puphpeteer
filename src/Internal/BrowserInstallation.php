...
final class BrowserInstallation
{
    ...
    public function __construct(string $packageRoot)
    {
        ...
        if ($cache === false || $cache === '') { $cache = '.chrome'; }
    ...
    public static function skipDownload(): bool
    {
        ...
        if ($executable !== false && $executable !== '') { return true; }
        foreach (['PUPPETEER_SKIP_DOWNLOAD', 'PUPPETEER_CHROME_SKIP_DOWNLOAD', 'PUPPETEER_SKIP_CHROME_DOWNLOAD'] as $key) {
            if (in_array(strtolower(getenv($key) ?: ''), ['1', 'true', 'yes'], true)) { return true; }
        }
        return false;
    }
    ...
    {
        if (self::skipDownload()) { return null; }
        ...
        if ($lock === false) { throw new \RuntimeException('Cannot open Chrome installation lock'); }
        $temporary = null;
        try {
            if (!flock($lock, LOCK_EX)) { throw new \RuntimeException('Cannot lock Chrome cache'); }
            if (is_file($this->executable()) && is_executable($this->executable())) { return $this->executable(); }
            ...
            if (!mkdir($temporary)) { throw new \RuntimeException('Cannot create Chrome download directory'); }
            $archive = $temporary . '/chrome.zip';
            $source = @fopen($this->url(), 'rb', false, stream_context_create(['http' => ['timeout' => 120]]));
            if ($source === false) { throw new \RuntimeException('Cannot download Chrome: ' . $this->url()); }
            try {
                $target = fopen($archive, 'wb');
                if ($target === false) { throw new \RuntimeException('Cannot write Chrome archive'); }
                try {
                    if (stream_copy_to_stream($source, $target) === false) { throw new \RuntimeException('Chrome download failed'); }
                } finally { fclose($target); }
            } finally { fclose($source); }
            $this->extract($archive, $temporary . '/unpacked');
            if (!rename($temporary . '/unpacked', $this->directory())) { throw new \RuntimeException('Cannot publish Chrome installation'); }
            return $this->executable();
        } finally {
            if ($temporary !== null && is_dir($temporary)) { self::removeDirectory($temporary); }
            flock($lock, LOCK_UN);
            fclose($lock);
        }
    }
    ...
    public function extract(string $archive, string $destination): void
    {
        ...
        if (!is_resource($process)) { throw new \RuntimeException('Install unzip to extract Chrome'); }
        fclose($pipes[0]);
        $code = proc_close($process);
        if ($code !== 0) { throw new \RuntimeException('Chrome extraction failed (requires unzip): ' . (file_get_contents($log) ?: '')); }
    ...
    private static function removeDirectory(string $path): void
    {
        ...
        foreach (new \FilesystemIterator($path) as $file) { if ($file instanceof \SplFileInfo) { self::removeDirectory($file->getPathname()); } }
        rmdir($path);
    }
}
