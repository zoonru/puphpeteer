...
Эта версия **находится в разработке и ещё не выпущена**. Сгенерированные обёртки покрывают часть API Puppeteer; поддержка `puppeteer-extra` и stealth запланирована.
...
См. также готовые к запуску [примеры](examples/).
...
`puppeteer-extra` и `puppeteer-extra-plugin-stealth` **пока не поддерживаются**. Старая настройка `js_extra` отклоняется. Прежде чем включить эту возможность, нужно проверить в QuickJS hooks плагинов, требования к Node API и отдельные stealth evasions.
...
На macOS расширение может иметь суффикс `.dylib`. `PHP_BIN` переопределяет исполняемый файл PHP для runner. Smoke запускает браузерные сценарии и все три примера. Бенчмарк пока рассчитан на macOS и измеряет PHP-нагрузку отдельно от Chrome и runner. Подробнее — [устройство QuickJS и проверки](docs/quickjs.md).
...
3. **Расширение:** закрепить прямой мост, `dispatch`, контракт типов, лимиты очередей, освобождение ресурсов и безопасность Fibers.
4. **Runtime:** довести отмену, таймауты, события, транспорт и lifecycle объектов и браузера.
5. **Плагины:** адаптировать и проверить hooks puppeteer-extra/stealth, evasions, страницы, frames и popup до фиксации архитектуры.
6. **Выпуск:** тесты совместимости, аварийные и длительные сценарии, утечки, повторяемые бенчмарки и реальная нагрузка.

Сейчас CI проверяет unit-тесты, Psalm, генерацию API и JS-сборку. Воспроизводимая сборка расширения и браузерная интеграция в CI ещё не реализованы. Исторические отчёты в [docs/benchmarks](docs/benchmarks/) описывают предыдущие снимки проекта.
