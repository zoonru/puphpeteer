...
`PUPPETEER_CHROME_SKIP_DOWNLOAD=true`. Для совместимости также принимается
`PUPPETEER_SKIP_CHROME_DOWNLOAD=true`. Эти переменные влияют на установочный
скрипт; для запуска укажите `PUPPETEER_EXECUTABLE_PATH` или параметры удалённого
подключения.
...
описан в [разделе о проверках QuickJS](docs/quickjs.md).
