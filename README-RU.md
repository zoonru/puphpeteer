...
Статические JS-методы вызываются через существующий PHP-объект. Если экземпляра нет, `getStaticClass()` возвращает типизированную обёртку конструктора в том же QuickJS-контексте:
...
$page->getStaticClass(Locator::class)
    ...
    ->click();
// Через уже созданный Locator: $locator->race([$locator, $other])->click();
```

Getter доступен также у `$puppeteer`: до `launch()`/`connect()` он создаёт контекст будущего соединения, после — использует последнее соединение. Не смешивайте объекты разных соединений. Для конкретного браузера используйте getter у `$browser` или `$page`.

Свойства читаются и записываются напрямую (`$object->property = $value`), если upstream разрешает запись. IDE видит типы и PHP 8.4 accessors. Совместимые перегрузки объединяются; Symbol-методы и Symbol-события не поддерживаются.
