...
function boundaryCheck(bool $value, string $message): void { if (!$value) { throw new RuntimeException($message); } }
...
try {
    ...
    $start = hrtime(true);
    for ($i = 0; $i < 3; ++$i) { $page->evaluate('42'); }
...
foreach (['socket', 'crash'] as $failure) {
    ...
    try {
        ...
        Amp\delay(0.05);
        if ($failure === 'crash') { $process->kill(); }
        else { (new ReflectionProperty($client, 'socket'))->getValue($client)->close(); }
        foreach ($pending as $future) {
            try { $future->await(new TimeoutCancellation(2)); throw new LogicException('Pending operation survived transport failure'); }
            catch (Amp\CancelledException $error) { throw new RuntimeException('Operation hung after transport failure', previous: $error); }
        ...
        if ($failure === 'socket') {
            ...
            try { boundaryCheck($reconnected->newPage()->evaluate('42') === 42, 'New connection cannot use surviving Chrome'); }
...
try {
    ...
    $browser->close();
    boundaryCheck(file_get_contents($profile . '/user-sentinel') === 'keep', 'User profile was deleted');
} finally { Nesk\Puphpeteer\Tests\Support\Shared\ProcessRunner::removeDirectory($profile); }
