...
try {
    ...
    if (!preg_match('/401|403/', $error->getMessage())) {
        throw $error;
    }
}
...
try {
    ...
    if (42 !== $page->evaluate('42')) {
        throw new RuntimeException('Browserless request failed');
    }
    $pending = async(fn () => $page->evaluate('new Promise(() => {})'));
    try {
        $pending->await(new TimeoutCancellation(6));
        throw new LogicException('Server session timeout did not terminate pending call');
    } catch (CancelledException $error) {
        throw new RuntimeException('Client deadline elapsed before Browserless timeout', previous: $error);
    } catch (RuntimeException $error) {
        if (!preg_match('/clos|disconnect|socket/i', $error->getMessage())) {
            throw $error;
        }
    }
    if ($browser->connected) {
        throw new RuntimeException('Expired browserless session remains connected');
    }
} finally {
    $browser->disconnect();
}
