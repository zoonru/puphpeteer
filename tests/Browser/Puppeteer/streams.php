...
if (false === $file) {
    throw new RuntimeException('Cannot create PDF fixture');
}
try {
    ...
    try {
        while (($chunk = $stream->read()) !== null) {
            if (strlen($chunk) > 65536) {
                throw new RuntimeException('Oversized stream chunk');
            }
            $pdf .= $chunk;
        }
    } finally {
    ...
    if (42 !== $page->evaluate('6 * 7')) {
        throw new RuntimeException('Stream close damaged browser connection');
    }
    if (!str_starts_with(buffer($page->createPDFStream()), '%PDF-')) {
