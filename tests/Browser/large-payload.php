...
try {
    ...
    foreach ([2097151, 2097153, 4194321, 8388639, 12582943] as $size) {
        ...
        foreach (await([async(fn() => $page->evaluate($fn, 'A')), async(fn() => $page->evaluate($fn, 'B'))]) as $i => $value) {
            LargePayload::verify(LargePayload::digest($value), $expected . ($i === 0 ? 'A' : 'B'), 'Concurrent result ' . $size);
        }
        if ($page->evaluate('6 * 7') !== 42) { throw new RuntimeException('Large result damaged connection'); }
