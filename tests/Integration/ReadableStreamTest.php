...
final class ReadableStreamTest extends TestCase
{
    ...
    public function testChunksAreBoundedExactAndYieldToTheEventLoop(): void
    {
        ...
        try {
            ...
            } finally { EventLoop::cancel($timer); }
    ...
    public function testCancellationClosesOnlyStreamAndRejectsConcurrentRead(): void
    {
        ...
        try {
            ...
            delay(0.01);
            try { $stream->read(); self::fail('Expected PendingReadError'); }
            catch (PendingReadError) { self::assertTrue(true); }
            $cancellation->cancel();
            try { $read->await(); self::fail('Expected cancellation'); }
            catch (CancelledException) { self::assertTrue(true); }
            delay(0);
            self::assertTrue($stream->isClosed());
    ...
    public function testSourceErrorsAndTransportShutdownReachReader(): void
    {
        ...
        try {
            foreach (['error' => 'source failure', 'invalid' => 'Uint8Array'] as $kind => $message) {
                ...
                try { $stream->read(); self::fail('Expected stream error'); }
                catch (StreamException $error) { self::assertStringContainsString($message, $error->getMessage()); }
                self::assertTrue($stream->isClosed());
            }
            $stream = $client->call(-1, 'stream', ['pending'])->await();
            $read = async(fn() => $stream->read());
            delay(0.01);
            $client->close();
            try { $read->await(); self::fail('Expected transport error'); }
            catch (StreamException $error) { self::assertStringContainsString('closed', $error->getMessage()); }
        } finally { $client->close(); }
    }
}
