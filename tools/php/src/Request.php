...
final class Request
{
    /** @return array{root:string,classes:list<ClassSpec>,knownMemberIds:list<string>} */
    public static function decode(string $json): array
    {
        ...
        foreach ($value['classes'] as $class) {
            ...
            foreach ($class['members'] as $member) {
                ...
                self::optionalStrings($member, ['returnType', 'returnDocType', 'type', 'docType']);
        ...
        $known = $value['knownMemberIds'] ?? [];
        if (!is_array($known) || !array_is_list($known)) {
            throw new InvalidArgumentException('knownMemberIds must be a list');
        }
        foreach ($known as $id) {
            if (!is_string($id)) {
                throw new InvalidArgumentException('knownMemberIds must contain strings');
            }
        }
        /** @var list<ClassSpec> $classes Validated structurally above; field semantics are validated during AST creation. */
        $classes = $value['classes'];
        return ['root' => $value['root'], 'classes' => $classes, 'knownMemberIds' => $known];
    }
