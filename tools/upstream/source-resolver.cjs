...
async function resolveSources(options = {}) {
    ...
    const packageSubdirectory = 'packages/puppeteer';
    ...
    const installedMatches = () => {
        try {
            ...
            if (name === 'puppeteer' && readJson(path.join(root, 'node_modules/puppeteer-core/package.json')).version !== version) return false;
