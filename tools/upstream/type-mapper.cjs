...
function createTypeMapper({types, config, classMap, requireClass, report}) {
    ...
    function map(type, id, context = {}) {
        ...
        if (['Promise', 'PromiseLike'].includes(name) && args.length) {
            ...
            return result('\\Amp\\Future', `\\Amp\\Future<${value.psalm === 'void' ? 'null' : value.psalm}>`);
        }
        ...
        if (classMap.has(name)) {
            ...
            return result(fqcn, fqcn + (args.length ? `<${args.map(arg => map(arg, id, context).psalm).join(', ')}>` : ''));
        }
        if (type.kind === 'function') {
            ...
            return result('callable', `callable(${parameters.join(', ')}):${parenthesize(returns.psalm)}`);
        }
